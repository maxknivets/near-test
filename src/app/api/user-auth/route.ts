...
export async function POST(request: Request) {
  ...
  let { mes, non, acc, pub, sig } = body;
  mes = Buffer.from(mes, "base64").toString("utf8");
  non = Buffer.from(non, "base64").toString("utf8");
  acc = Buffer.from(acc, "base64").toString("utf8");
  pub = Buffer.from(pub, "base64").toString("utf8");
  sig = Buffer.from(sig, "base64").toString("utf8");

  const isAuthed = await authenticate(mes, acc, pub, non, sig);
